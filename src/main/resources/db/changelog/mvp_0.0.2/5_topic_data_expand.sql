...
create or replace function validate_alias() returns trigger as $alias_trg$
    begin
        if exists(select 1 from notification_topic where name = new.readable_alias) then
...
update notification_topic set readable_alias = '' where notification_topic.readable_alias is null;
